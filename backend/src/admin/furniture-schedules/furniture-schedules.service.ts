...
import {
  FurnitureScheduleNotifyProject,
  FurnitureScheduleNotifyService,
} from './furniture-schedule-notify.service';
import {
...
export { FURNITURE_SCHEDULE_STALE_DAYS } from './furniture-schedule.shared';

@Injectable()
export class FurnitureSchedulesService {
  constructor(
    private readonly prisma: PrismaService,
    private readonly notify: FurnitureScheduleNotifyService,
    private readonly importService: FurnitureSchedulesImportService,
  ) {}
  ...
  async create(dto: CreateFurnitureScheduleProjectDto, createdById: string) {
    ...
    const status = dto.status ?? FurnitureScheduleProjectStatus.NEW;
    const contractDate = dto.contractDate ? parseDateOnly(dto.contractDate) : null;
    const pauseStartDate = dto.pauseStartDate ? parseDateOnly(dto.pauseStartDate) : null;
    const pauseResumeDate = dto.pauseResumeDate ? parseDateOnly(dto.pauseResumeDate) : null;
    const project = await this.prisma.furnitureScheduleProject.create({
      data: {
        status,
        contractNumber,
        installationContractNumber: emptyToNull(dto.installationContractNumber) ?? null,
        appliancesContractNumber: emptyToNull(dto.appliancesContractNumber) ?? null,
        repairInfo: emptyToNull(dto.repairInfo) ?? null,
        reviewInfo: emptyToNull(dto.reviewInfo) ?? null,
        ...
        closedAt: status === FurnitureScheduleProjectStatus.CLOSED ? new Date() : null,
        createdById,
  ...
  async findAll(params: {
    ...
    const where: Prisma.FurnitureScheduleProjectWhereInput = {
      ...(status ? { status } : {}),
      ...
      ...(search
        ? {
            OR: [
              ...
              { installationContractNumber: { contains: search, mode: 'insensitive' } },
              { appliancesContractNumber: { contains: search, mode: 'insensitive' } },
              ...
              { repairInfo: { contains: search, mode: 'insensitive' } },
            ],
          }
        : {}),
    };

    const rows = await this.prisma.furnitureScheduleProject.findMany({
      where,
      include: PROJECT_INCLUDE,
  ...
  async findMy(
    userId: string,
    params?: { status?: FurnitureScheduleProjectStatus; search?: string },
  ) {
    const search = params?.search?.trim();
    const rows = await this.prisma.furnitureScheduleProject.findMany({
      where: {
  ...
  async assertCanAccessProject(projectId: string, userId: string, role: string) {
    ...
    const project = await this.prisma.furnitureScheduleProject.findUnique({
      where: { id: projectId },
      select: { installer: { select: { userId: true } } },
    });
    if (!project) throw new NotFoundException(`Furniture schedule project ${projectId} not found`);
  ...
  async findOne(id: string) {
    ...
    if (!project) throw new NotFoundException(`Furniture schedule project ${id} not found`);
    return withDerived(project);
  }

  async update(id: string, dto: UpdateFurnitureScheduleProjectDto, actorUserId: string) {
    await this.findOne(id);
    const data: Prisma.FurnitureScheduleProjectUpdateInput = {
      updatedBy: { connect: { id: actorUserId } },
    };

    if (dto.status !== undefined) {
      data.status = dto.status;
      data.closedAt = dto.status === FurnitureScheduleProjectStatus.CLOSED ? new Date() : null;
    }
    if (dto.contractNumber !== undefined) {
      data.contractNumber = emptyToNull(dto.contractNumber) ?? null;
    }
    if (dto.installationContractNumber !== undefined) {
      data.installationContractNumber = emptyToNull(dto.installationContractNumber) ?? null;
    }
    if (dto.appliancesContractNumber !== undefined) {
      data.appliancesContractNumber = emptyToNull(dto.appliancesContractNumber) ?? null;
    }
    if (dto.repairInfo !== undefined) data.repairInfo = emptyToNull(dto.repairInfo) ?? null;
    if (dto.reviewInfo !== undefined) data.reviewInfo = emptyToNull(dto.reviewInfo) ?? null;
    ...
    if (dto.pauseStartDate !== undefined) {
      data.pauseStartDate = dto.pauseStartDate ? parseDateOnly(dto.pauseStartDate) : null;
    }
    if (dto.pauseResumeDate !== undefined) {
      data.pauseResumeDate = dto.pauseResumeDate ? parseDateOnly(dto.pauseResumeDate) : null;
    }
    ...
    const updated = await this.prisma.furnitureScheduleProject.update({
      where: { id },
      data,
      include: PROJECT_DETAIL_INCLUDE,
    });
  ...
  async setStatus(id: string, status: FurnitureScheduleProjectStatus, actorUserId: string) {
    return this.update(id, { status }, actorUserId);
  }

  async remove(id: string) {
    await this.findOne(id);
    await this.prisma.furnitureScheduleProject.delete({ where: { id } });
    return { ok: true };
  }

  async addEntry(projectId: string, dto: CreateFurnitureScheduleEntryDto, createdById: string) {
    ...
    await this.prisma.furnitureScheduleEntry.create({
      data: {
        ...
        kind: dto.kind ?? FurnitureScheduleEntryKind.WEEKLY,
        text,
        createdById,
      },
    });
  ...
  async updateEntry(
    projectId: string,
    entryId: string,
    dto: UpdateFurnitureScheduleEntryDto,
    actorUserId: string,
  ) {
    const entry = await this.prisma.furnitureScheduleEntry.findFirst({
  ...
  async removeEntry(projectId: string, entryId: string, actorUserId: string) {
    ...
    if (!entry) throw new NotFoundException('Запись не найдена');
    await this.prisma.furnitureScheduleEntry.delete({ where: { id: entryId } });
    await this.prisma.furnitureScheduleProject.update({
      where: { id: projectId },
      data: { updatedById: actorUserId },
    });
