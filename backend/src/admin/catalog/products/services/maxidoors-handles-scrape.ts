...
export type MaxidoorsHandlesListingItem = {
  productKey: string;
  name: string;
  price: number | null;
...
export type MaxidoorsHandlesDetailData = {
  title: string;
  price: number | null;
...
export function parseListingHtml(html: string): MaxidoorsHandlesListingItem[] {
  const $ = cheerio.load(html);
  const items: MaxidoorsHandlesListingItem[] = [];
...
export function detectLastListPage(html: string): number {
  const $ = cheerio.load(html);
  let max = 1;
  $('a[href*="/product-category/furnitura/ruchki"]').each((_, a) => {
...
export async function scrapeAllHandleListings(
...
): Promise<MaxidoorsHandlesListingItem[]> {
  const firstUrl = `${MAXIDOORS_BASE}${MAXIDOORS_HANDLES_LIST_PATH}`;
  const firstHtml = await fetchHtml(firstUrl);
  const lastPage = detectLastListPage(firstHtml);
  const all: MaxidoorsHandlesListingItem[] = [];
  const seen = new Set<string>();
  ...
  for (let page = 2; page <= lastPage; page++) {
    await sleep(delayMs);
    const url = `${MAXIDOORS_BASE}${MAXIDOORS_HANDLES_LIST_PATH}page/${page}/`;
    try {
...
export function parseDetailHtml(html: string): MaxidoorsHandlesDetailData {
  const $ = cheerio.load(html);
...
export async function fetchDetail(
  url: string,
  delayMs: number,
): Promise<MaxidoorsHandlesDetailData> {
...
export async function downloadMaxidoorsImagesToUploads(
  ...
  for (let i = 0; i < remoteUrls.length; i++) {
    ...
    try {
      ...
      const filename = `md-h-${safeKey}-${i}${ext}`;
...
export function buildSeoFields(productName: string, categoryName: string) {
  ...
  const keywords = [productName, categoryName, 'ручка', 'фурнитура', 'Максидорс']
    .map((k) => k.trim())
    .filter(Boolean);
