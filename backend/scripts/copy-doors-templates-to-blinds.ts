/**
 * Копирует шаблоны по умолчанию DOORS → BLINDS для вкладок:
 * Договор, Акт приёма, Накладная, Согласие.
 *
 * Локально: npx ts-node -r tsconfig-paths/register scripts/copy-doors-templates-to-blinds.ts
 */
...
const TARGET_KIND = 'BLINDS' as const;
...
async function main() {
  ...
  for (const tabId of TABS_TO_COPY) {
    ...
    const newId = `seed-blinds-from-doors-${tabId}`;
  ...
  console.log(`✅ BLINDS: скопированы шаблоны DOORS → ${copied.join('; ') || 'ничего'}`);
}
