/**
 * Скрипт для импорта дверей серии "Аргус" из HTML-файла экспорта Битрикс
 * Использование: npx ts-node scripts/import-doors-argus.ts
 */
...
async function importDoors() {
  ...
  // Находим или создаём подкатегорию "Входные двери Аргус"
  const category = await prisma.category.upsert({
    where: { slug: 'entrance-doors-argus' },
    update: {
