/**
 * Скрипт для импорта дверей из HTML-файла экспорта Битрикс
 * Использование: npx ts-node scripts/import-doors.ts <путь_к_файлу.xls>
 */
...
function parseHtmlTable(html: string): DoorData[] {
  ...
  const rowRegex = /<tr><td>([^<]*)<\/td><td[^>]*>([^<]*)<\/td><td[^>]*>([^<]*)<\/td><td>([^<]*)<\/td><td>([^<]*)<\/td><td>([^<]*)<\/td><td[^>]*>([^<]*)<\/td><td>([^<]*)<\/td><td[^>]*>([^<]*)<\/td><td[^>]*>([^<]*)<\/td><td[^>]*>([^<]*)<\/td><td[^>]*>([^<]*)<\/td><td>([^<]*)<\/td><td>([^<]*)<\/td><td[^>]*>([^<]*)<\/td><td[^>]*>([^<]*)<\/td><td[^>]*>([^<]*)<\/td><td>([^<]*)<\/td><td[^>]*>([^<]*)<\/td><td>([\s\S]*?)<\/td><td>([^<]*)<\/td><td[^>]*>([^<]*)<\/td><td[^>]*>([^<]*)<\/td><td[^>]*>([^<]*)<\/td><td[^>]*>([^<]*)<\/td><td[^>]*>([^<]*)<\/td><td[^>]*>([^<]*)<\/td><td[^>]*>([^<]*)<\/td><td[^>]*>([^<]*)<\/td><td[^>]*>([^<]*)<\/td><td[^>]*>([^<]*)<\/td><td[^>]*>([^<]*)<\/td><td[^>]*>([^<]*)<\/td><\/tr>/gi;

  // Простой парсинг: разбиваем по </tr><tr>
...
async function importDoors(filePath: string) {
  ...
  // Находим или создаём подкатегорию "Входные двери ТТ XL / XXL"
  ...
  // Удаляем старые тестовые товары из этой категории
...
async function main() {
  ...
  if (!filePath) {
    ...
    console.log('Использование: npx ts-node scripts/import-doors.ts <путь_к_файлу.xls>');
    process.exit(1);
  }

  try {
    await importDoors(filePath);
  } catch (error) {
