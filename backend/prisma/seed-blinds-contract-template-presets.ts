...
const TAB_TEMPLATE_FILES: Record<(typeof LIBRARY_TABS)[number], string> = {
  /** Дефолты как у «Двери» (договор / акт / накладная / согласие); памятка — своя. */
...
export async function seedBlindsContractTemplatePresets(): Promise<void> {
  const mode = (process.env.BLINDS_TEMPLATES_SEED_MODE ?? 'fill-missing').trim();
  ...
  console.log(`✅ Библиотека шаблонов BLINDS (${activeByTab}); режим ${mode}`);
}
