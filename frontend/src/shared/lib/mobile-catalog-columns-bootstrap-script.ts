...
export function buildSiteBootstrapScript(
  ...
  return `(function(){var h=location.hostname.toLowerCase();var isQuiz=${quizHostCheck};if(isQuiz){document.documentElement.setAttribute('data-theme','light');}else{var t=localStorage.getItem('theme');document.documentElement.setAttribute('data-theme',t==='dark'?'dark':'light');}var k=${JSON.stringify(MOBILE_CATALOG_COLUMNS_STORAGE_KEY)};var v=localStorage.getItem(k);var cols=v==='1'?1:v==='2'?2:${serverDefault};if(cols===2){document.documentElement.setAttribute('data-mobile-catalog-columns','2');}else{document.documentElement.removeAttribute('data-mobile-catalog-columns');}})();`;
}
