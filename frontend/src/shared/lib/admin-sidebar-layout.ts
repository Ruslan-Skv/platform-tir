...
export function buildAdminSidebarBootstrapScript(): string {
  return `try{var w=parseInt(localStorage.getItem(${JSON.stringify(ADMIN_SIDEBAR_WIDTH_STORAGE_KEY)})||'',10);if(isNaN(w)||w<${ADMIN_SIDEBAR_MIN_WIDTH}||w>${ADMIN_SIDEBAR_MAX_WIDTH})w=${ADMIN_SIDEBAR_DEFAULT_WIDTH};var c=localStorage.getItem(${JSON.stringify(ADMIN_SIDEBAR_COLLAPSED_STORAGE_KEY)})==='1';var o=c?${ADMIN_SIDEBAR_COLLAPSED_WIDTH}:w;var r=document.documentElement;r.style.setProperty('--admin-sidebar-width',w+'px');r.style.setProperty('--admin-sidebar-offset',o+'px');if(c)r.setAttribute('data-admin-sidebar-collapsed','');else r.removeAttribute('data-admin-sidebar-collapsed');var uiRaw=localStorage.getItem(${JSON.stringify(ADMIN_SIDEBAR_UI_PREFS_STORAGE_KEY)});var desktopLayout='list';var mobileLayout='list';if(uiRaw){var ui=JSON.parse(uiRaw);if(ui.desktopLayout==='grid2')desktopLayout='grid2';if(ui.mobileLayout==='grid3')mobileLayout='grid3';}r.setAttribute('data-admin-sidebar-desktop-layout',desktopLayout);r.setAttribute('data-admin-sidebar-mobile-layout',mobileLayout);}catch(e){}`;
}

export const ADMIN_SIDEBAR_UI_PREFS_STORAGE_KEY = 'admin.sidebar.uiPrefs';
