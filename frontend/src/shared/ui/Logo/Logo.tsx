...
export type LogoProps = {
  ...
  /** Только иконка, без текстовой части логотипа. */
  markOnly?: boolean;
  /** Без ссылки (например, внутри другой ссылки в админке). */
  unlinked?: boolean;
  ariaLabel?: string;
};
...
export const Logo: React.FC<LogoProps> = ({
  ...
  const svg = <LogoSvg className={className} markOnly={markOnly} />;

  if (unlinked) {
    return (
      <span className={[styles.logoLink, linkClassName].filter(Boolean).join(' ')}>{svg}</span>
    );
  }
  ...
  return (
    <Link
      href={href}
      className={[styles.logoLink, linkClassName].filter(Boolean).join(' ')}
