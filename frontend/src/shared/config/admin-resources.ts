/**
 * Список ресурсов админки (синхронизирован с backend resources.config.ts).
 * resourceId — ключ в admin_resource_permissions; path — маршрут в UI админки.
 */
...
export const ADMIN_RESOURCES: AdminResourceItem[] = [
  ...
  { id: 'admin.crm.dp', label: 'ДП', path: '/admin/crm/dp' },
...
const ROUTES_BY_PATH_LENGTH = [...ADMIN_RESOURCES].sort((a, b) => b.path.length - a.path.length);

const LABEL_BY_ID = new Map(ADMIN_RESOURCES.map((r) => [r.id, r.label]));

/** Определяет resourceId по URL страницы админки (самый длинный совпадающий path). */
export function resolveAdminResourceIdByPathname(pathname: string): string | null {
...
export function getAdminResourceLabel(resourceId: string): string {
  const known = LABEL_BY_ID.get(resourceId);
  if (known) return known;

  if (resourceId.startsWith('admin.knowledge.category.')) {
...
/** Страница создания/редактирования — нужен уровень EDIT. */
export function isAdminEditRoute(pathname: string): boolean {
  if (/\/(new|create)(\/|$)/.test(pathname) || /\/edit(\/|$)/.test(pathname)) {
    return true;
  }

  if (pathname === '/admin/measurements') return false;
  if (/^\/admin\/measurements\/[^/]+$/.test(pathname)) return true;

  if (pathname === '/admin/recruitment/blank') return false;
  if (pathname.startsWith('/admin/recruitment/analytics')) return false;
  if (pathname === '/admin/recruitment') return false;
  if (/^\/admin\/recruitment\/[^/]+$/.test(pathname)) return true;

  if (pathname === '/admin/orders') return false;
  if (/^\/admin\/orders\/service-orders\/[^/]+$/.test(pathname)) return true;
  if (/^\/admin\/orders\/[^/]+$/.test(pathname)) return true;

  if (pathname === '/admin/crm/supplier-settlements') return false;
  if (/^\/admin\/crm\/supplier-settlements\/[^/]+$/.test(pathname)) return true;

  if (/^\/admin\/contract-documents\/contracts\/[^/]+$/.test(pathname)) return true;
  if (/^\/admin\/contract-documents\/repair\/[^/]+$/.test(pathname)) return true;
  if (/^\/admin\/contract-documents\/estimates\/[^/]+$/.test(pathname)) return true;

  if (/^\/admin\/catalog\/categories\/[^/]+\/attributes$/.test(pathname)) return true;
  if (
    /^\/admin\/catalog\/suppliers\/[^/]+$/.test(pathname) &&
    pathname !== '/admin/catalog/suppliers'
  ) {
    return true;
  }
  if (/^\/admin\/partners\/[^/]+$/.test(pathname) && pathname !== '/admin/partners') return true;

  if (
    /^\/admin\/catalog\/categories\/[^/]+$/.test(pathname) &&
...
export function adminEditRouteRedirectTarget(pathname: string): string | null {
  if (pathname.endsWith('/new') || pathname.endsWith('/create')) {
    return pathname.replace(/\/(new|create)$/, '');
  }

  const idEditMatch = pathname.match(/^(.+)\/[^/]+\/edit$/);
  if (idEditMatch) return idEditMatch[1];

  const editMatch = pathname.match(/^(.+)\/edit$/);
  if (editMatch) return editMatch[1];

  if (/^\/admin\/measurements\/[^/]+$/.test(pathname)) return '/admin/measurements';
  if (/^\/admin\/recruitment\/[^/]+$/.test(pathname)) return '/admin/recruitment';
  if (/^\/admin\/orders\/service-orders\/[^/]+$/.test(pathname))
    return '/admin/orders/service-orders';
  if (/^\/admin\/orders\/[^/]+$/.test(pathname)) return '/admin/orders';
  if (/^\/admin\/crm\/supplier-settlements\/[^/]+$/.test(pathname)) {
    return '/admin/crm/supplier-settlements';
  }
  if (/^\/admin\/contract-documents\/contracts\/[^/]+$/.test(pathname)) {
    return '/admin/contract-documents/contracts';
  }
  if (/^\/admin\/contract-documents\/repair\/[^/]+$/.test(pathname)) {
    return '/admin/contract-documents/contracts';
  }
  if (/^\/admin\/contract-documents\/estimates\/[^/]+$/.test(pathname)) {
    return '/admin/contract-documents/estimates';
  }
