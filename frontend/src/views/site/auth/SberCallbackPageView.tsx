...
function SberCallbackContent() {
  ...
  const processCallback = useCallback(async () => {
    if (errorParam) {
      ...
      setErrorMessage(
        errorParam === 'access_denied'
          ? 'Вы отменили вход через Сбер ID'
          : 'Ошибка авторизации Сбер ID'
      );
      return;
    }
    ...
    try {
      const res = await apiFetch(`${getApiBaseUrl()}/auth/sber/callback`, {
        method: 'POST',
      ...
      if (!res.ok) {
        setStatus('error');
        setErrorMessage(data.message || 'Не удалось войти через Сбер ID');
        return;
      }
      ...
      <div className={styles.container}>
        <div className={styles.card}>
          <div className={styles.header}>
            <h1 className={styles.title}>Вход через Сбер ID</h1>
          </div>
...
export function SberCallbackPageView() {
  return (
    <Suspense
      fallback={
        <div className={styles.container}>
