...
import {
  type FurnitureScheduleImportResult,
  importFurnitureScheduleExcel,
...
type Props = {
  isOpen: boolean;
  onClose: () => void;
  onImported: (result: FurnitureScheduleImportResult) => void;
};

export function FurnitureScheduleImportModal({ isOpen, onClose, onImported }: Props) {
  const inputRef = useRef<HTMLInputElement>(null);
  const [file, setFile] = useState<File | null>(null);
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [result, setResult] = useState<FurnitureScheduleImportResult | null>(null);
  ...
  const runImport = async () => {
    ...
    setError(null);
    try {
      const res = await importFurnitureScheduleExcel(file, [2025, 2026]);
      setResult(res);
      onImported(res);
    } catch (err) {
    ...
    <Modal isOpen={isOpen} onClose={handleClose} title="Импорт из Excel / Google" size="md">
      <div data-modal-form data-modal-density="compact">
        <p data-modal-form-hint className={styles.formHintFlush}>
          Загрузите выгрузку Google Sheets или Excel «План-график». Импортируются колонки недель
