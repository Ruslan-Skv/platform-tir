...
import { FURNITURE_STATUS_LABELS } from '../shared/furniture-schedules';
import { buildRepairObjectGroups } from '../shared/furnitureObjectGroups';
import { FurnitureProjectForm } from './FurnitureProjectForm';
import { FurnitureScheduleImportModal } from './FurnitureScheduleImportModal';
import { FurnitureSchedulesGanttTimeline } from './FurnitureSchedulesGanttTimeline';
import { FurnitureSchedulesGroupedList } from './FurnitureSchedulesGroupedList';
import { FurnitureSchedulesRulesInfoTip } from './FurnitureSchedulesRulesInfoTip';
import type {
  FurnitureDeadlineFilter,
  FurnitureSchedulesPageModel,
} from './hooks/useFurnitureSchedulesPage';
...
const DEADLINE_FILTERS: Array<{ key: FurnitureDeadlineFilter; label: string }> = [
  { key: 'ALL', label: 'Все' },
...
export function FurnitureSchedulesPageView({ model }: { model: FurnitureSchedulesPageModel }) {
  const {
    statusFilter,
    setStatusFilter,
    deadlineFilter,
  ...
  return (
    <div className={`${cdBase.page} ${cdWorkspace.pageWide} ${cdHub.contractsListPage}`}>
          ...
          <div className={cdHub.contractsHeaderTitleRow}>
            <div className={cdHub.contractsHeaderTitleCluster}>
              <div className={cdHub.contractsListHeaderTitleGroup}>
                <h1 className={cdHub.title}>План-график мебели</h1>
                <FurnitureSchedulesRulesInfoTip />
              </div>
        ...
        <div className={cdHub.contractsListFiltersStack}>
          <div className={cdHub.contractsListChipRow} role="group" aria-label="Статус">
            ...
            {(
              ...
              >
                {FURNITURE_STATUS_LABELS[key]} ({statusCounts[key]})
              </button>
            ))}
          </div>
      ...
      <div className={styles.viewPanels}>
        ...
        >
          <FurnitureSchedulesGanttTimeline
            items={items}
            loading={loading}
