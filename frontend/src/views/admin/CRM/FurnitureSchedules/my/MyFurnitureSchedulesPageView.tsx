...
import type { MyFurnitureSchedulesPageModel } from './hooks/useMyFurnitureSchedulesPage';
...
export function MyFurnitureSchedulesPageView({ model }: { model: MyFurnitureSchedulesPageModel }) {
  const {
    statusFilter,
    setStatusFilter,
    search,
  ...
  const columns = [
    {
      key: 'contract',
      title: 'Договор',
      render: (item: FurnitureScheduleProject) => (
    ...
    },
    {
      key: 'sum',
      title: 'Стоимость',
      render: (item: FurnitureScheduleProject) => formatMoney(item.contractSum),
    },
    {
      key: 'latest',
          ...
          <div className={cdHub.contractsHeaderTitleRow}>
            <div className={cdHub.contractsHeaderTitleCluster}>
              <div className={cdHub.contractsListHeaderTitleGroup}>
                <h1 className={cdHub.title}>Моя мебель</h1>
              </div>
