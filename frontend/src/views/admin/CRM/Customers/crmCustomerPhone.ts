/** Подсказка и плейсхолдер для полей телефона клиента CRM. */
export const CRM_PHONE_PLACEHOLDER = '+7(000)-000-00-00';

export const CRM_PHONE_FORMAT_HINT = 'Формат: +7(000)-000-00-00 или 8(000)-000-00-00';
...
export function normalizeCrmPhoneDigits(phone: string): string | null {
...
export function isValidCrmPhone(phone: string): boolean {
  const t = phone.trim();
  if (!t) return false;
...
export function formatCrmPhoneInput(raw: string): string {
...
/** Отображение сохранённого номера в читаемом виде. */
export function formatCrmPhoneDisplay(phone: string): string {
  const t = phone.trim();
  if (!t) return '';
  if (PHONE_FORMAT_PLUS7_RE.test(t) || PHONE_FORMAT_8_RE.test(t)) return t;

  const digits = normalizeCrmPhoneDigits(t);
  if (!digits) return t;
...
/** Канонический формат перед сохранением (после валидации). */
export function normalizeCrmPhoneForStorage(phone: string): string {
  const formatted = formatCrmPhoneDisplay(phone.trim());
  if (!formatted || !isValidCrmPhone(formatted)) return '';
  return formatted;
}

export function normalizeCrmPhonesList(phones: readonly string[]): string[] {
  const out: string[] = [];
  for (const raw of phones) {
    const normalized = normalizeCrmPhoneForStorage(raw);
    if (!normalized) continue;
    const d = digitsOnlyPhone(normalized);
    if (out.some((x) => digitsOnlyPhone(x) === d)) continue;
    out.push(normalized);
  }
  return out;
}

export function formatCrmPhoneOrDash(phone: string | null | undefined): string {
  const t = phone?.trim();
  if (!t) return '—';
