...
import { type WaybillTask, getWaybillTrash, restoreWaybillTask } from '@/shared/api/admin-waybills';
...
import {
  WAYBILL_TRASH_RETENTION_NOTICE,
  waybillTrashPermanentDeleteAtIso,
} from '../shared/waybillTrashRetention';
import {
  formatTimeRange,
  formatUserLabel,
...
export interface WaybillTrashModalProps {
  isOpen: boolean;
  onClose: () => void;
  onRestored?: () => void;
}

export function WaybillTrashModal({ isOpen, onClose, onRestored }: WaybillTrashModalProps) {
  const [searchInput, setSearchInput] = useState('');
  const [searchQuery, setSearchQuery] = useState('');
  const [page, setPage] = useState(1);
  const [rows, setRows] = useState<WaybillTask[]>([]);
  ...
  const loadTrash = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await getWaybillTrash({
      ...
      >
        <div className={styles.searchRow} data-modal-form-grid>
          <div data-modal-form-group data-modal-span>
            <label htmlFor="waybill-trash-search">Поиск в корзине</label>
            <input
              id="waybill-trash-search"
              type="search"
              placeholder="Задание, заказчик, направление, № договора…"
              value={searchInput}
              ...
              <article key={row.id} className={styles.entry}>
                <div className={styles.entryMain}>
                  <div className={styles.entryMeta}>
                    ...
                    <span className={styles.entrySub}>{row.taskText}</span>
                    {(() => {
                      const customer = resolveWaybillCustomerFields(row);
                      const line = [
                        ...
                        .filter(Boolean)
                        .join(' · ');
                      return line ? <span className={styles.entrySub}>{line}</span> : null;
                    })()}
                    ...
                    {row.deletedAt &&
                    (row.permanentDeleteAt || waybillTrashPermanentDeleteAtIso(row.deletedAt)) ? (
                      <span className={styles.entryDeleted}>
        ...
        <div data-modal-footer-info data-modal-tone="info" role="status">
          ...
          <span data-modal-footer-info-text>
            Удалённые задания скрыты из путевого листа. Восстановление вернёт задание на его дату.
            Удалять можно только задания «В плане», и только автор записи.{' '}
            {WAYBILL_TRASH_RETENTION_NOTICE}
          </span>
        </div>
      </form>
