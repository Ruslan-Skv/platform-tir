...
type UseAdminStickySaveButtonParams = {
  enabled?: boolean;
  saving: boolean;
  loading?: boolean;
  pageHeaderRef: React.RefObject<HTMLDivElement | null>;
  onSave: () => void;
  scrollToTopOnSave?: boolean;
};

export function useAdminStickySaveButton({
  enabled = true,
  saving: _saving,
  loading = false,
  pageHeaderRef,
  ...
  const scrollPageToTop = useCallback(() => {
    ...
    // eslint-disable-next-line prefer-const -- assigned after finishScroll declaration
  ...
  }, [getPageScrollTop, pageHeaderRef, scrollToTopOnSave]);
  ...
  }, [loading, enabled, measureSaveButtonPinnedTop, applySaveButtonPinState]);
...
export type AdminStickySaveButtonState = ReturnType<typeof useAdminStickySaveButton>;
