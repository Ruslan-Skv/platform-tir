...
const HELP = {
  ...
  note: 'Настройки колокольчика в шапке, звука и браузерных / push-уведомлений. Супер-администратор задаёт, какие события видит каждая роль. Email, Telegram и MAX настраиваются отдельно в «Каналах уведомлений о заявках».',
} as const;
...
const PUSH_ITEMS = [
  '«Уведомления на рабочем столе и в приложении (PWA)» — запрос разрешения браузера и подписка на push.',
  'Интервал проверки задаёт, как часто обновляется лента колокольчика.',
  'После сохранения с включённым desktop/push браузер может запросить разрешение — его нужно разрешить.',
] as const;
