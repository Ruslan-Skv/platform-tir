...
function normalizeLikeTemplateLibrary(raw: string): string {
  return unifyContractDocumentTypographyInHtml(repairContractTemplateStructureInHtml(raw));
}
...
describe('contractTemplateAppendixRef', () => {
  ...
  it('converts nested heading markup to left-aligned appendix paragraph', () => {
    ...
    doc.body.innerHTML = repairContractTemplateStructureInHtml(NESTED_ACT_HEADER_HTML);
  ...
  it('repairContractAppendixRefInHtml converts act subheading to h2 title', () => {
    const out = repairContractAppendixRefInHtml(NESTED_ACT_HEADER_HTML);
    expect(out).toContain('contractDocTitle');
    expect(out).not.toMatch(/<h3[^>]*>[\s\S]*?Акт\s+прием/i);
  });

  it('repairContractTemplateStructureInHtml converts appendix line', () => {
    const out = repairContractTemplateStructureInHtml(NESTED_ACT_HEADER_HTML);
