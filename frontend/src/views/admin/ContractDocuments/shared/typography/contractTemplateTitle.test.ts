...
<h1 class="repairAddendumHeaderTitle">Дополнительное соглашение №1</h1>
</div>
<p>${intro.textContent}</p>
  ...
  it('does not treat addendum header sub as contract title', () => {
    ...
    sub.className = 'repairAddendumHeaderSub';
...
<div class="repairAddendumHeaderBlock">
<h1 class="repairAddendumHeaderTitle">Дополнительное соглашение №1</h1>
<p class="repairAddendumHeaderSub">${sub.textContent}</p>
</div></div>`;
    normalizeContractTitleInDom(wrap);
    const subAfter = wrap.querySelector('.repairAddendumHeaderSub');
  ...
  it('repairContractTitleBlock flattens nested title in table cell', () => {
  ...
  it('flattens nested h1>h3 contract title and keeps contractDocTitle', () => {
    ...
    doc.body.innerHTML = repairContractTemplateStructureInHtml(WINDOWS_CONTRACT_TITLE_CELL);
