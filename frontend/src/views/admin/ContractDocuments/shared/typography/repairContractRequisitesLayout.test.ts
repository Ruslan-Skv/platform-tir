...
describe('repairContractRequisitesLayout', () => {
  it('detects requisites table with ИСПОЛНИТЕЛЬ and signatures', () => {
    ...
    expect(isRepairRequisitesSignaturesTableHtml(table)).toBe(true);
  });
  ...
  it('toolbar insert block has aligned signatures and embedded flag', () => {
    const html = buildRepairContractRequisitesInsertHtmlForToolbar();
