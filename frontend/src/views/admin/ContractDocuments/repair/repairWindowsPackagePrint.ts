...
import { isRepairActA4PreviewTab } from './repairActTwinCopiesOnOnePageHtml';
import { type RepairDocumentTabId, isRepairAddendumTab } from './repairDocumentTabs';
...
/** Вкладки редактора, для которых в пакете «Окна» включается единый компактный профиль печати. */
export function shouldUseWindowsPackageCompactPrint(
  ...
  return (
    ...
    tab === 'memo' ||
    tab === 'productionLog' ||
    isRepairActA4PreviewTab(tab) ||
    isRepairAddendumTab(tab) ||
    tab === 'workOrder' ||
...
export function resolveRepairEditorPrintOptions(
  packageKind: ContractDocumentPackageKind,
  tab: RepairDocumentTabId,
  formData: unknown
