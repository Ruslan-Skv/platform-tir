...
function repairNestedH2MemoShellInDom(docPrint: HTMLElement): void {
...
function repairAdjacentMemoHeadingsInDom(docPrint: HTMLElement): void {
  const doc = docPrint.ownerDocument;
...
function repairCombinedMemoHeadingInDom(docPrint: HTMLElement): void {
  const doc = docPrint.ownerDocument;
...
function repairMemoHeadingWithBlockChildrenInDom(docPrint: HTMLElement): void {
  const doc = docPrint.ownerDocument;
...
function repairNestedH2MemoShellInHtmlString(html: string): string {
  let out = html;
  let prev = '';
  while (prev !== out) {
    prev = out;
...
export function repairBrokenMemoHeadingNestingInDom(root: ParentNode): void {
  ...
  repairNestedH2MemoShellInDom(docPrint);
  repairAdjacentMemoHeadingsInDom(docPrint);
  repairCombinedMemoHeadingInDom(docPrint);
  repairMemoHeadingWithBlockChildrenInDom(docPrint);
  repairContractAppendixRefInDom(docPrint);
}

export function repairBrokenMemoHeadingNestingInHtml(html: string): string {
  if (!/<h2\b/i.test(html) || !MEMO_TITLE_RE.test(html)) return html;
  const stringFixed = repairNestedH2MemoShellInHtmlString(html);
  ...
  repairBrokenMemoHeadingNestingInDom(container);
