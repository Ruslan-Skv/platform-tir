...
import { applyRepairContractDiscountToAmount } from '../../directions/repair/estimates/repairContractDiscount';
...
const ESTIMATE_TAB_COMPACT = `${cdEstimateTab.estimateTabCompact} ${cdWindows.estimateTabCompact}`;
const ESTIMATE_BLOCK = `${cdDataTab.blockData} ${cdWindows.blockData}`;
const ESTIMATE_DATA_COMPACT = `${cdEstimateTab.dataCompact} ${cdDataTab.dataCompact}`;
const ESTIMATE_FORM_GRID = `${cdDataTab.formGrid} ${cdWindows.formGrid}`;
const ESTIMATE_SECTION_CARD = `${cdTemplates.sectionCard} ${cdEstimateTab.sectionCard}`;
const ESTIMATE_SECTION_FIELDS = `${cdTemplates.sectionFields} ${cdEstimateTab.sectionFields}`;
const ESTIMATE_SECTION_HEADER = `${cdTemplates.estimateSectionHeader} ${cdEstimateTab.estimateSectionHeader}`;
const ESTIMATE_SECTION_TITLE = `${cdEstimateTab.sectionTitle}`;
const ESTIMATE_SECTION_TITLE_MAIN = `${cdTemplates.estimateSectionTitle} ${cdEstimateTab.estimateSectionTitle}`;
const ESTIMATE_HINT = `${cdDocPreview.hint} ${cdTemplates.hint}`;
const ESTIMATE_TAB_HINT = `${ESTIMATE_HINT} ${cdEstimateTab.estimateTabHint}`;
...
export function PackageEstimateTab({
  ...
  return (
    <div
      className={`${ESTIMATE_BLOCK} ${ESTIMATE_DATA_COMPACT} ${ESTIMATE_TAB_COMPACT}${isProductDirectionPackage ? ` ${cdWindows.windowsContractTabTypography}` : ''}`}
    >
          ...
          <div className={ESTIMATE_SECTION_FIELDS}>
            ...
            >
              ...
              <aside className={cdEstimateTab.estimateAttachedColumn}>
                <div className={cdEstimateTab.estimateAttachedColumnTitle}>Прикреплённые</div>
                {(form.estimate.selectedPresetIds?.length ?? 0) > 0 ? (
            ...
            !(form.estimate.selectedPresetIds ?? []).every(
              ...
              <p className={`${ESTIMATE_TAB_HINT} ${cdEstimateTab.estimateTabHintFullWidth}`}>
            ...
            <div
              className={`${cdEstimateTab.field} ${cdEstimateTab.fieldSpanAll} ${cdEstimateTab.estimateSheetField}`}
            >
            ...
            <p
              className={`${ESTIMATE_HINT} ${cdEstimateTab.fieldSpanAll} ${cdEstimateTab.estimateTabHintFullWidth}`}
            >
