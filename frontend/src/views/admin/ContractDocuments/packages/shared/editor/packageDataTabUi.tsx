...
export function PackageDataSectionLockInline({ title }: { title: string }) {
  return (
    <span
      className={cdDataTab.repairDataSectionLockInline}
      role="img"
      aria-label={title}
      title={title}
    >
...
export function PackageDataPartySectionCollapseButton({
  ...
  return (
    <button
      type="button"
      className={`${cdWorkspace.secondaryBtn} ${cdDataTab.repairDataPartySectionCollapseBtn}`}
    ...
    >
      {expanded ? (
        <ChevronUpIcon className={cdDataTab.repairDataPartySectionCollapseIcon} aria-hidden />
      ) : (
        <ChevronDownIcon className={cdDataTab.repairDataPartySectionCollapseIcon} aria-hidden />
      )}
    </button>
  );
}
