...
import { computeContractDeadlineFromWorkPeriodStart } from '../../directions/repair/form/repairContractWorkPeriod';
...
export function getSignedAddendumOrdinals(form: RepairPackageFormData): number[] {
  const count = clampRepairAddendumSlotCount(form.addendumSlotCount);
  const ordinals: number[] = [];
  for (let i = 0; i < count; i++) {
    const slot = form.addendumSlots[i];
