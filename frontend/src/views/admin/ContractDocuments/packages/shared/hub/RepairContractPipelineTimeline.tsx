...
import type { RepairPipelineModel, RepairPipelineStepVisualState } from './repairContractPipeline';
import { REPAIR_WORK_START_MIN_CONTRACT_PAY_PCT } from './repairContractPipeline';
import type { RepairContractPackageHubState } from './useRepairContractPackageHub';

type RepairContractPipelineTimelineProps = {
  hub: RepairContractPackageHubState;
  pipeline: RepairPipelineModel;
...
function stepStateClass(state: RepairPipelineStepVisualState): string {
  switch (state) {
    case 'completed':
...
function payPctToneClass(pct: number | null): string {
  ...
  if (pct >= REPAIR_WORK_START_MIN_CONTRACT_PAY_PCT) return hubStyles.pipelinePayBadgeLime;
  return hubStyles.pipelinePayBadgeYellow;
}

export function RepairContractPipelineTimeline({
  hub,
  pipeline,
...
}: RepairContractPipelineTimelineProps) {
                ...
                !pipeline.repairWorkStarted ? (
                  <div className={hubStyles.pipelineStepActions}>
                    <button
                      ...
                      title={
                        !pipeline.workStartPaymentReady
                          ? `Нужна оплата по договору не менее ${REPAIR_WORK_START_MIN_CONTRACT_PAY_PCT}% (сейчас ${pipeline.contractPaidPct ?? 0}%)`
                          : actionsTitle
                      }
                      onClick={() => {
