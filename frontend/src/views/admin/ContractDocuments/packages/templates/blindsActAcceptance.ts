/** Шаблон «Акт приёма» для направления «Жалюзи» (изделия + монтаж). */
export const blindsTemplateActAcceptance = `
<div class="docPrint">
  <h1>Акт приёма</h1>
  <p>к договору № {{contract.number}} от {{contract.date}}</p>
  <p>
    Исполнитель передал, а Заказчик принял жалюзи в соответствии со спецификацией (приложение к договору),
