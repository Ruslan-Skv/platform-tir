...
import {
  ...
  templateTrashPermanentDeleteAtIso,
} from '../../templateTrashRetention';
...
interface TemplateTrashModalProps {
  isOpen: boolean;
  onClose: () => void;
  onRestored?: () => void;
}

export function TemplateTrashModal({ isOpen, onClose, onRestored }: TemplateTrashModalProps) {
  const [searchInput, setSearchInput] = useState('');
  const [searchQuery, setSearchQuery] = useState('');
  const [page, setPage] = useState(1);
  const [rows, setRows] = useState<ContractTemplatePresetTrashRow[]>([]);
  ...
  const loadTrash = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await getContractDocumentTemplatePresetsTrash({
  ...
  const handleRestore = async (id: string) => {
    ...
    setError(null);
    try {
      await restoreContractTemplatePreset(id);
      onRestored?.();
      await loadTrash();
    } catch (err) {
      ...
      >
        <div className={styles.searchRow} data-modal-form-grid>
          <div data-modal-form-group data-modal-span>
            <label htmlFor="template-trash-search">Поиск в корзине</label>
            <input
              id="template-trash-search"
              type="search"
              placeholder="Название, тип документа…"
              value={searchInput}
              ...
              <article key={row.id} className={styles.entry}>
                <div className={styles.entryMain}>
                  <div className={styles.entryMeta}>
                    ...
                    <span className={styles.entrySub}>{row.tabLabel}</span>
                    <span className={styles.entryDeleted}>
                      Удалён {formatCrmDateTimeLocale(row.deletedAt)} ·{' '}
                      {formatPackageUserLabel(row.deletedBy)}
                    </span>
                    {row.permanentDeleteAt || templateTrashPermanentDeleteAtIso(row.deletedAt) ? (
                      <span className={styles.entryDeleted}>
        ...
        <div data-modal-footer-info data-modal-tone="info" role="status">
          ...
          <span data-modal-footer-info-text>
            Шаблоны в корзине скрыты из библиотеки и из выбора в пакете «Ремонт».{' '}
            {TEMPLATE_TRASH_RETENTION_NOTICE}
          </span>
        </div>
      </form>
