...
export function listActiveLibraryTemplatesForTab(
  ...
  return items.filter((it) => {
    ...
    return packageLibraryTemplateTabIdFromPreset(it.tabId) === tab;
  });
}
...
export function fixMisassignedProductLibraryPresetTabId(
  ...
  if (packageLibraryTemplateTabIdFromPreset(preset.tabId) === 'contract') {
    return { ...preset, tabId: 'memo' };
  }
  return preset;
}

export function fixMisassignedProductLibraryPresets(
  items: ContractTemplatePreset[],
  kind: ContractDocumentPackageKind
): ContractTemplatePreset[] {
  return items.map((it) => fixMisassignedProductLibraryPresetTabId(it, kind));
}

export function canAutosaveLibraryTemplatePreset(
  items: ContractTemplatePreset[],
  editingId: string,
  activeTab: PackageLibraryTemplateTabId
): boolean {
  if (!editingId.trim()) return false;
  const preset = items.find((it) => it.id === editingId);
  if (!preset) return true;
  const presetTab = packageLibraryTemplateTabIdFromPreset(preset.tabId);
