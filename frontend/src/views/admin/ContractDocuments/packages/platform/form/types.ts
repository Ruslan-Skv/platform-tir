...
export interface PackageFormData {
  ...
  /**
   * Картинки с чертежами (вкладка «Чертежи», направление «Потолки») — относительные URL.
   * Порядок = порядок прикрепления; максимум 10 шт.
   */
  drawingPhotoUrls: string[];
  /**
   * Скрины чеков об оплате от клиентов (QR/банковский перевод) — относительные URL.
   * Порядок = порядок прикрепления; максимум 12 шт.
   */
  paymentProofPhotoUrls: string[];
  /**
   * Ручная связь пакета с замером (вкладка «Замер»), id из CRM → Замерам.
   * Дублирует автоматическую цепочку «расчёт из замера», когда расчёт создан иначе.
   */
