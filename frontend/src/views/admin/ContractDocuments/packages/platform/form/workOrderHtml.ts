...
import { buildWindowsWorkOrderComputed } from '../../families/product-like/print/productWorkOrder';
...
} from './packageContractDiscount';
import type { PackageEstimateBlock, PackageFormData } from './types';
...
function buildWorkOrderComputed(
  ...
  const contractDiscountPercent = parsePackageContractDiscountPercent(contractDiscountPercentRaw);
  const rooms: WorkOrderRoom[] = (snapshot?.rooms ?? []).map((room) => {
    const lines = room.lines.map((line) => {
      const afterDiscount = applyPackageContractDiscountToAmount(
        line.amount,
        contractDiscountPercent
      );
      const afterDiscountPrice = applyPackageContractDiscountToAmount(
        line.price,
        contractDiscountPercent
      );
      return {
  ...
  const afterDiscountTotal = applyPackageContractDiscountToAmount(
    originalTotal,
    contractDiscountPercent
  );
