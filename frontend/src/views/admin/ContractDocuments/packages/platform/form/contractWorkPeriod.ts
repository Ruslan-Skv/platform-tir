export const DEFAULT_PACKAGE_CONTRACT_WORK_PERIOD_DAYS = 60;
export const DEFAULT_PRODUCT_CONTRACT_WORK_PERIOD_DAYS = 90;

export function formatPackageWorkPeriodDays(days: number): string {
  const n = Math.trunc(days);
  if (!Number.isFinite(n) || n < 1) return String(DEFAULT_PACKAGE_CONTRACT_WORK_PERIOD_DAYS);
  return String(n);
}

export function parsePackageWorkPeriodInput(raw: string): number | null {
  const s = raw.trim();
...
export function normalizePackageWorkPeriodInput(raw: string): string {
...
export function computeContractDeadlineFromWorkPeriodStart(
  ...
  const workingDays = parsePackageWorkPeriodInput(workPeriodDaysRaw ?? '');
...
function isLegacyDefaultWorkPeriodDays(
  days: number,
  packageKind: 'REPAIR' | 'WINDOWS' | 'DOORS'
): boolean {
  if (packageKind === 'WINDOWS' || packageKind === 'DOORS') {
    return (
      days === DEFAULT_PRODUCT_CONTRACT_WORK_PERIOD_DAYS ||
      days === DEFAULT_PACKAGE_CONTRACT_WORK_PERIOD_DAYS
    );
  }
  return days === DEFAULT_PACKAGE_CONTRACT_WORK_PERIOD_DAYS;
}

export function isPackageWorkPeriodManualFlag(value: unknown): boolean {
  return value === true;
}
...
export function resolvePackageWorkPeriodForForm(
  ...
  defaultDays: number,
  packageKind: 'REPAIR' | 'WINDOWS' | 'DOORS' = 'REPAIR',
  workPeriodIsManual = false
): { value: string; autofill: boolean } {
  const trimmed = (stored ?? '').trim();
  if (workPeriodIsManual) {
    const parsedManual = parsePackageWorkPeriodInput(trimmed);
    if (parsedManual !== null) {
      return { value: trimmed, autofill: false };
    }
    return {
      value: formatPackageWorkPeriodDays(defaultDays),
      autofill: true,
    };
  }
  const parsed = parsePackageWorkPeriodInput(trimmed);
  if (parsed !== null) {
