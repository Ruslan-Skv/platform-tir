...
import type { PackageCustomerBlock, PackageFormData } from './types';
 ...
 */
export type PackageTemplatePreviewCustomerKind = PackageCustomerBlock['type'];

export function buildPackageTemplatePreviewFallbackData(
  executorProfile: ExecutorRequisiteProfile | null | undefined,
  signatoryProfile: ContractSignatoryProfile | null | undefined,
  customerKind: PackageTemplatePreviewCustomerKind = 'PERSON'
): PackageFormData {
  ...
  const customerByKind: PackageCustomerBlock =
 ...
 * Пример для превью шаблонов: `buildPackageTemplatePreviewFallbackData` в библиотеке.
 */
export function mergePackageFormWithPreviewFallback(
  form: PackageFormData,
  _fallback: PackageFormData
): PackageFormData {
  return form;
}
