import type { PackageCustomerBlock } from './packageForm';

/** Контекст документа для выбора формулировки заказчика в `{{customer.fullName}}`. */
export type PackageCustomerTemplateContext =
  | 'contract'
  | 'act'
  | 'cashOrder'
  | 'paymentInvoice'
...
export function packageCustomerTemplateContextFromTab(
  templateTab: string | undefined
): PackageCustomerTemplateContext {
...
export interface PackageCustomerTemplateFields {
...
export function buildPackageCustomerTemplateFields(
  customer: PackageCustomerBlock
): PackageCustomerTemplateFields {
...
export function packageCustomerContextualFullName(
  fields: PackageCustomerTemplateFields,
  context: PackageCustomerTemplateContext
): string {
  switch (context) {
    case 'contract':
...
export type PackageCustomerForTemplate = PackageCustomerBlock & PackageCustomerTemplateFields;

export function enrichPackageCustomerForTemplate(
  customer: PackageCustomerBlock,
  context: PackageCustomerTemplateContext
): PackageCustomerForTemplate {
  const fields = buildPackageCustomerTemplateFields(customer);
  const contextualFullName = packageCustomerContextualFullName(fields, context);
  return {
    ...customer,
    ...fields,
