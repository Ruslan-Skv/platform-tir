/**
 * Группы плейсхолдеров для вставки в шаблон договора (пути совпадают с `packageForm.ts`).
 * Для заказчика без жирного в предпросмотре: `{{customer.field|plain}}` (см. `applyTemplate`).
 * `meta.currentDate` подставляется из `packageFormForTemplate` (дата на момент рендера).
 * `contract.contractCost` / `productsCost` / `worksCost` — из сметы, спецификации (Окна) и скидки по договору.
 */

export interface PackageContractPlaceholderItem {
  path: string;
  label: string;
}

export interface PackageContractPlaceholderGroup {
  title: string;
  items: PackageContractPlaceholderItem[];
}

export const PACKAGE_CONTRACT_PLACEHOLDER_GROUPS: PackageContractPlaceholderGroup[] = [
  {
