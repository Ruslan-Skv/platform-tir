...
export function readPackagePaymentBasisOptions(): string[] {
...
export function appendPackagePaymentBasisOption(
  ...
  const prev = readPackagePaymentBasisOptions();
  if (prev.includes(t)) {
  ...
  const list = [...prev, t];
  writePackagePaymentBasisOptions(list);
  return { ok: true, list };
}
