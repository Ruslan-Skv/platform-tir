...
function buildInstallerFinalWorkOrderBodyHtml(
  doc: PackagePerInstallerWorkOrder,
...
export function buildFinalWorkOrderPrintEmbedHtml(
  ...
  return `<div class="${docPrintClass}"><div class="estimateA4DocPrintEmbed estimateRoomsEmbed packageFinalWorkOrderPrint">
...
export function buildAllFinalWorkOrdersPrintHtml(
  ...
  return chunks
    .map(
      (html, index) =>
        `<div class="packageWorkOrderHubPrintChunk"${index > 0 ? ' style="page-break-before:always;break-before:page;"' : ''}>${html}</div>`
    )
    .join('\n');
}
