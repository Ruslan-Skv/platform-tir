...
const WO_TAB_COMPACT = `${cdEstimateTab.estimateTabCompact} ${cdProduct.estimateTabCompact}`;
const WO_BLOCK = `${cdDataTab.blockData} ${cdProduct.blockData}`;
const WO_DATA_COMPACT = `${cdEstimateTab.dataCompact} ${cdDataTab.dataCompact}`;
const WO_FORM_GRID = `${cdDataTab.formGrid} ${cdProduct.formGrid}`;
...
export function PackageWorkOrdersHubPanels({ panelTab }: PackageWorkOrdersHubPanelsProps) {
  const ctx = usePackageWorkOrderHub();
  const {
    isWindowsPackage,
    form,
    updateWorkOrder,
