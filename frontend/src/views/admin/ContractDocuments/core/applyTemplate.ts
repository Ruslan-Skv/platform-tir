...
import {
  ...
  htmlHasEmbeddedRequisitesSignatures,
  isPackageRequisitesSignaturesTableHtml,
  normalizeRequisitesBlockTypography,
} from './typography/packageContractRequisitesLayout';
...
function ensureRequisitesTableClass(html: string): string {
  ...
  for (;;) {
    ...
    if (isPackageRequisitesSignaturesTableHtml(block) && !block.includes(REQUISITES_TABLE_CLASS)) {
...
function ensureFinalSignaturesRow(html: string, flat: Record<string, string>): string {
  ...
  if (htmlHasEmbeddedRequisitesSignatures(html)) return html;
 ...
 * Служебные поля из `packageFormForTemplate`, например `{{meta.currentDate}}` (дд.мм.гггг).
 */
