((this.workbox = this.workbox || {}),
  (this.workbox.strategies = (function (t, e, s, r, n, a, i, o, c) {
    'use strict';
    try {
    ...
    class l {
      ...
      async fetch(t) {
        ...
        try {
          ...
          for (const e of this.iterateCallbacks('fetchDidSucceed'))
            t = await e({ event: s, request: a, response: t });
          return t;
        } catch (t) {
          throw (
            n &&
      ...
      async cacheMatch(t) {
        const e = h(t);
        let s;
        const { cacheName: r, matchOptions: n } = this.ht,
          a = await this.getCacheKey(e, 'read'),
      ...
      async cachePut(t, s) {
        ...
        if (!s)
          throw new e.WorkboxError('cache-put-with-no-response', { url: i.getFriendlyURL(o.url) });
        const l = await this.xt(s);
        if (!l) return !1;
        const { cacheName: w, matchOptions: u } = this.ht,
          f = await self.caches.open(w),
          d = this.hasCallback('cacheDidUpdate'),
          p = d ? await r.cacheMatchIgnoreParams(f, o.clone(), ['__WB_REVISION__'], u) : null;
        try {
          await f.put(o, d ? l.clone() : l);
        } catch (t) {
          if (t instanceof Error)
            throw ('QuotaExceededError' === t.name && (await a.executeQuotaErrorCallbacks()), t);
        }
      ...
      async getCacheKey(t, e) {
        ...
        if (!this.vt[s]) {
          ...
          for (const t of this.iterateCallbacks('cacheKeyWillBeUsed'))
            r = h(await t({ mode: e, request: r, event: this.event, params: this.params }));
          this.vt[s] = r;
        }
        return this.vt[s];
      }
      ...
      *iterateCallbacks(t) {
      ...
      waitUntil(t) {
        return (this.Et.push(t), t);
      }
      async doneWaiting() {
        let t;
        for (; (t = this.Et.shift()); ) await t;
      }
      destroy() {
        this.bt.resolve(null);
      }
      async xt(t) {
        let e = t,
          s = !1;
    ...
    class w {
      ...
      async Rt(t, s, r) {
        ...
        try {
          ...
          if (e instanceof Error)
            for (const a of t.iterateCallbacks('handlerDidError'))
              if (((n = await a({ error: e, event: r, request: s })), n)) break;
          if (!n) throw e;
        }
      ...
      async Wt(t, e, s, r) {
        ...
        try {
          (await e.runCallbacks('handlerDidRespond', { event: r, request: s, response: n }),
            await e.doneWaiting());
        } catch (t) {
    ...
    return (
      (t.CacheFirst = class extends w {
        async _handle(t, s) {
          ...
          if (!n) throw new e.WorkboxError('no-response', { url: t.url, error: r });
          return n;
        }
      }),
      ...
      (t.NetworkFirst = class extends w {
        ...
        async _handle(t, s) {
          ...
          let a;
          if (this.Ot) {
            const { id: e, promise: i } = this.Ut({ request: t, logs: r, handler: s });
            ((a = e), n.push(i));
          }
          const i = this.Ct({ timeoutId: a, request: t, logs: r, handler: s });
          n.push(i);
          const o = await s.waitUntil(
            (async () => (await s.waitUntil(Promise.race(n))) || (await i))()
          );
          if (!o) throw new e.WorkboxError('no-response', { url: t.url });
          return o;
        }
        Ut({ request: t, logs: e, handler: s }) {
          let r;
          return {
        ...
        async Ct({ timeoutId: t, request: e, logs: s, handler: r }) {
          let n, a;
          try {
            a = await r.fetchAndCachePut(e);
          } catch (t) {
            t instanceof Error && (n = t);
          }
          return (t && clearTimeout(t), (!n && a) || (a = await r.cacheMatch(e)), a);
        }
      }),
      ...
      (t.StaleWhileRevalidate = class extends w {
        ...
        async _handle(t, s) {
          ...
          if (!a) throw new e.WorkboxError('no-response', { url: t.url, error: n });
          return a;
        }
      }),
      (t.Strategy = w),
