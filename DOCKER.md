...
| mailhog       | 1025, 8025 | Перехват писем (SMTP + Web UI) |

Письма (восстановление пароля, заказ на email) при локальной сборке отправляются в **MailHog**. Все письма можно посмотреть в браузере: **http://localhost:8025**. Бэкенд подключается к SMTP по имени сервиса `mailhog:1025`; отдельная настройка в `.env` не требуется.

**Сид БД при запуске через Docker:** бэкенд в контейнере использует БД `platform_tir` по адресу `postgres:5432`. Надёжный способ — запустить сид **внутри контейнера** (тот же `DATABASE_URL`, что и у API):
...
Сервис `seed` в `docker-compose.yml` один раз выполняет `npm run prisma:seed` с подключением к `postgres:5432/platform_tir`. После выполнения пользователи (например, `user@example.com`) будут в той же БД, что и API; письма восстановления пароля появятся в MailHog.
